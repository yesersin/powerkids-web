...
class CalendarAgendaState extends State<CalendarAgenda> with TickerProviderStateMixin {
        ...
        child: ScrollablePositionedList.builder(
            padding: _dates.length < 5
                ? EdgeInsets.symmetric(
                    horizontal: MediaQuery.of(context).size.width * (5 - _dates.length) / 10)
                          ...
                          image: isSelected
                              ? DecorationImage(
                                  image: widget.selectedDayLogo ??
                                      MemoryImage(base64.decode(uri.split(',').last)),
                          ...
                          children: [
                            ...
                            Text(
                              ...
                              style: TextStyle(
                                  fontSize: 22.0,
                                  color:
                                      isSelected ? widget.selectedDateColor : widget.dateColor,
  ...
  _generateDates() {
    ...
    DateTime first =
        DateTime.parse("${widget.firstDate.toString().split(" ").first} 00:00:00.000");

    DateTime last =
        DateTime.parse("${widget.lastDate.toString().split(" ").first} 23:00:00.000");

    DateTime basicDate = DateTime.parse("${first.toString().split(" ").first} 12:00:00.000");

    List<DateTime> listDates = List.generate((last.difference(first).inHours / 24).round(),
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                height: 6,
                decoration: BoxDecoration(
                    borderRadius: BorderRadius.circular(3.0), color: Color(0xFFE0E0E0)),
              ),
              SizedBox(
                height: 20.0,
              ),
  ...
  _selectedDay() {
    DateTime getSelected =
        DateTime.parse("${_selectedDate.toString().split(" ").first} 00:00:00.000");

    _daySelectedIndex = _dates.indexOf(_dates.firstWhere((dayDate) =>
        DateTime.parse("${dayDate.toString().split(" ").first} 00:00:00.000") == getSelected));
  }
