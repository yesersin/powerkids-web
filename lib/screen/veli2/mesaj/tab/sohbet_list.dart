...
Widget sohbet({required COgretmen c}) {
  ...
  for (int i = 0; i < c.mesajSohbetList.length; i++) {
    if (c.mesajSohbetList[i].ogretmenId.id == cp.kullanici.data.id) {
      continue;
    }
    list.add(
      ModelMesajSohbet(
        adSoyad: c.mesajSohbetList[i].ogretmenId.adSoyad,
        id: c.mesajSohbetList[i].ogretmenId.id,
        profilResim: c.mesajSohbetList[i].ogretmenId.fotografUrl,
        notificationId: c.mesajSohbetList[i].ogretmenId.notificationId,
        guncelleme: c.mesajSohbetList[i].guncellemeZamani,
        okunmayan: c.mesajSohbetList[i].oOkunmayan.toString(),
        sonMesaj: c.mesajSohbetList[i].sonMesaj,
      ),
    );
  }
  List<Widget> w = [];
