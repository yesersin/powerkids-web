...
import 'package:com.powerkidsx/screen/ogretmen/anasayfa/etkinlikler/widget/etkinlik_ogrenci_list.dart';
...
import '../../../../const/etkinlik_tip.dart';
...
class _OgretmenEtkinliklerGirisState extends State<OgretmenEtkinliklerGiris> {
  ...
  Widget ogrenciKontrol() {
    if (widget.c.ogrenciList.value!.success != -1) {
      ...
      // for (var element in widget.c.ogrenciList.value!.data) {
      ...
      widget.c.etkinliklerOgrenciSecilen.value = List.generate(
        ...
        (index) => ModelEtkinlikEkleOgrenci(
          ...
          tercih: widget.c.seciliEtkinlik.value.secenekBir,
        ),
      );
    ...
    return FutureBuilder(
      ...
      builder: (BuildContext context, AsyncSnapshot<ModelSinifOgrencileri?> snapshot) {
        ...
        if (snapshot.data == null) {
          // toast(msg: hataMesaj);
          return Center(
              child: Text("Bu sınıfta öğrenci yok: " + hataMesaj, style: TextStyle()));
        }

        widget.c.etkinliklerOgrenciSecilen.value = List.generate(
            snapshot.data!.data.length,
            (index) => ModelEtkinlikEkleOgrenci(
                ogrenciId: snapshot.data!.data[index].id,
                ogrenci: snapshot.data!.data[index],
                tercih: widget.c.seciliEtkinlik.value.etkinlikAdi));
        Future.delayed(Duration(microseconds: 1), () {
          //obx hatası verdiği için yapıldı
            ...
            Container(
              ...
              child: Buton().mavi(
                  click: () async {
                    ...
                    listeBildirimGonder(
                        tip: '2', body: "Etkinlik Eklendi", pushBildirim: true);
  ...
  Future<void> etkinlikEkle() async {
    ...
    for (int i = 0; i < widget.c.ogrenciList.value!.data.length; i++) {
      ...
      // debugPrint("etkinlikAdi:" + widget.c.seciliEtkinlik.value.etkinlikAdi);
      // debugPrint("tercih:" + widget.c.etkinliklerOgrenciSecilen[i].tercih);
      // continue;
      ModelEtkinlikAkisAddCevap? cevap = await etkinlikAkisAdd(
        ...
        tercih: (widget.c.seciliEtkinlik.value.tip ==
                EtkinlikTip.input) //etkinlik input ise içindeki text al
            ? widget.c.etkinliklerOgrenciSecilen[i].textController.text
            : widget.c.etkinliklerOgrenciSecilen[i].tercih,
  ...
  void initState() {
    ...
    super.initState();
    widget.c.ogrenciList.value!.success = -1;
