...
Future<ModelDogumGunuKullanici?> getDogumGunuKullanici(
    {required String okulId, required String token}) async {
  http.Response? response = await postData(
      adres: ServiceAdres().kullaniciDogumGunu,
      body: {"okulId": okulId},
  ...
  if (!isStatus(response)) {
    ...
    try {
      return ModelDogumGunuKullanici.fromJson(jsonDecode(response!.body));
    } catch (e) {
